...
WorkflowData is the central data store for workflow execution. Values are keyed
by string and read back with type-specific accessors:
...
For type-safe access, declare a typed Key once and share it between the node
that produces a value and the nodes that consume it. Producer and consumer are
then checked at compile time:
...
See the runnable Example functions in this package's documentation, and the
