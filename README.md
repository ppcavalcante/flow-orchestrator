...
> **no stable (`v1`+) release**. The latest is **`v0.14.0-alpha`** (M15 — the decomposed,
> row-based `SQLiteStore` + incremental `O(N)` checkpointing + indexed visibility queries — on top
> of M12 saga / compensation (durable reverse-order rollback with `WithCompensation`, the
...
> currently **`v0.14.0-alpha`** — so the command above is correct. Pinning the exact version
> (`@v0.14.0-alpha`) is optional but recommended for reproducibility, and the API may change between
> alpha minors (see [STABILITY.md](STABILITY.md)). The in-code version (`pkg/workflow.Version`) reads
> `0.14.0-alpha`. See [CHANGELOG.md](CHANGELOG.md).
...
- **Latest release**: `v0.14.0-alpha` (the highest published tag; the `pkg/workflow.Version` marker on `main` reads `0.14.0-alpha`). Every tag is a pre-release, so `go get @latest` resolves to this; see the Versioning note under [Installation](#installation).
