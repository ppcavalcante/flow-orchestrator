...
> **no stable (`v1`+) release**. The latest is **`v0.22.4-alpha`** (M23 — Sealed Graph + Complete
...
> currently **`v0.22.4-alpha`** — so the command above is correct. Pinning the exact version
> (`@v0.22.4-alpha`) is optional but recommended for reproducibility, and the API may change between
...
- **Latest release**: `v0.22.4-alpha` (the highest published tag; the `pkg/workflow.Version` marker on `main` reads `0.22.4-alpha`). Every tag is a pre-release, so `go get @latest` resolves to this; see the Versioning note under [Installation](#installation).
