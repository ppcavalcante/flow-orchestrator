...
> **no stable (`v1`+) release**. The latest is **`v0.22.2-alpha`** (M23 — Sealed Graph + Complete
...
> the exact `N` branches with zero re-expansion, no determinism/replay tax, on any `Checkpointer`;
> the static-DAG executor is **0-diff** (the moat holds) and the core claim is machine-checked
> (TLA+ `ExactlyNSpawn` / `FanInWaitsForAll` + a genuine 2-proc kill-9 + a gopter arm) — on top of
> M20 — Scheduling + Concurrency
> Caps: opt-in durable cron / interval / one-shot schedules fired by an in-process poller, plus
> cross-process concurrency caps (“at most K of type X running”) — hand-rolled cron (zero new
> dependency), zero new entry point, fully additive — on top of M19 — Composition: opt-in
...
> reverse-order rollback with `WithCompensation`, the
> `Compensated` / `CompensationFailed` statuses, and a typed `SagaError`, crash-safe mid-rollback),
> M11 conditional branching
...
> currently **`v0.22.2-alpha`** — so the command above is correct. Pinning the exact version
> (`@v0.22.2-alpha`) is optional but recommended for reproducibility, and the API may change between
...
- **Latest release**: `v0.22.2-alpha` (the highest published tag; the `pkg/workflow.Version` marker on `main` reads `0.22.2-alpha`). Every tag is a pre-release, so `go get @latest` resolves to this; see the Versioning note under [Installation](#installation).
