...
// exportDemo wires the host SDK, drives library metrics activity, bridges the
// collector to OTel, and returns the collected+exported ResourceMetrics. It is a
// standalone function (not inlined into run) so the smoke test can assert on the
// exported instruments — the same "what ships is what is tested" shape every
// example in this suite follows.
func exportDemo(ctx context.Context) (metricdata.ResourceMetrics, error) {
	...
	if err != nil {
		return rm, fmt.Errorf("create stdout exporter: %w", err)
	}
	...
	//
	// metrics.NewConfig() is DISABLED by default (metrics are opt-in — the hot
	// path pays nothing unless you ask for them), so we explicitly enable it.
	// WithEnabled(true) keeps NewConfig's sampling rate of 1.0, so every
	// operation is recorded and the demo's exported counts are deterministic.
	// (metrics.ProductionConfig() is also enabled but samples at 0.01, which
	// would make an example's output non-reproducible.)
	...
	if err != nil {
		return rm, fmt.Errorf("create OTel bridge: %w", err)
	}
	...
	if err := reader.Collect(ctx, &rm); err != nil {
		return rm, fmt.Errorf("collect metrics: %w", err)
	}
	if err := exporter.Export(ctx, &rm); err != nil {
		return rm, fmt.Errorf("export metrics: %w", err)
	}
	return rm, nil
}
